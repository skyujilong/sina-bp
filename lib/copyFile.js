 ...
 * @param  {Object} qbInfo         svn相关信息对象
 * @param  {Object} webpackConfigInfo publicPath信息
 * @param  {String} projectName       项目名称
 * @return {[type]}                   修改后的文本
 */
function filterFileContent(fData, key, qbInfo, webpackConfigInfo, projectName) {
    switch (key) {
        case 'qb.html':
            ...
            fData = fData.replace('onlineSvnPathStr', qbInfo.svnUrl + '/assets/');
            //svn qb上线到的tag目录
            fData = fData.replace('tagPathStr', qbInfo.svnTagUrl);
            //标签名称  这里应该是文件的名字
            fData = fData.replace('tagNameStr', qbInfo.tagName);
            //标签内新增目录
            fData = fData.replace('addPathStr', qbInfo.qbAddPath);
            break;
        case 'config.js':
            fData = fData.replace('{{publicPath}}', webpackConfigInfo.devHost);
            fData = fData.replace('{{onlinePath}}', webpackConfigInfo.onLineHost);
            fData = fData.replace('{{onlineImgPath}}', webpackConfigInfo.onLineImgHost);
            break;
        case 'package.json':
 ...
 * @param  {Object} qbInfo         SVN相关配置
 * @param  {String} rootDir           生成项目的根路径
 * @return {Promise}                  返回promise对象
 */
function copyFile(webpackConfigInfo, qbInfo, rootDir) {
    let projectName = qbInfo.tagName;
    return co(function*() {
        let filesInfo = [];
        //获取filesInfo
        ...
        for (let fileInfo of filesInfo) {
            ...
            fileContent = filterFileContent(fileContent, fileInfo.fName, qbInfo, webpackConfigInfo, projectName);
            if (!fileContent) {
                continue;
            }
