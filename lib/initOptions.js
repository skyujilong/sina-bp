...
let argv = require('optimist').default({
    ...
    'onLineImgHost': 'http://test.sina.com.cn/' //可空
}).argv;
...
module.exports = function() {
    return co(function * () {
        ...
        if (argv.conf !== '') {
            ...
            //没有初始化 采用备用的
            if (!cfg.devHost) {
                cfg.devHost = urlEndSuff(argv.devHost, '/');
            }else{
                cfg.devHost = urlEndSuff(cfg.devHost,'/');
            }
            
            if (!cfg.onLineHost) {
                cfg.onLineHost = urlEndSuff(argv.onLineHost, '/');
            }else{
                cfg.onLineHost = urlEndSuff(cfg.onLineHost,'/');
            }

            if (!cfg.onLineImgHost) {
                cfg.onLineImgHost = urlEndSuff(argv.onLineImgHost, '/');
            }else{
                cfg.onLineImgHost = urlEndSuff(cfg.onLineImgHost,'/');
            }
        }
        //同时存在 name与conf字段的时候
        ...
        if (!argv.conf) {
            ...
            cfg.onLineImgHost = argv.onLineImgHost;
        }
